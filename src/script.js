...
async function fetchPokemon(pokemon, div) {
  ...
  if (!data.types[1]) {
    div.innerHTML += `<div id="id-${pokeId}" class="pokeCard card${type0}">
    <div class="infos">
    <p>${pokeId}. ${name}</p>
...
function searchPokemon() {
  ...
  if (rawText.length == 0) {
    ...
    const searchResult = arrayPokemons.filter((element) => {
      ...
      return (
        element.name.toLowerCase().includes(text) == true || text == element.id
      );
    });
    ...
    if (searchResult[0]) {
      ...
      searchDiv.innerHTML =
        '<p class="noresult">no result founds!</p><div id="alert"><i class="fa-solid fa-triangle-exclamation"></i></div>';
      searchDiv.classList.remove("hide");
    }
  }
}
